...
class Get_book_2_ChapterDataHandler extends HandlerNotNeedLogin_1.HandlerNotNeedLogin {
    async onGetData(req, res, datastr) {
        try {
            ...
            var selecterpage = await TConfigSelector_1.TConfigSelector.findByPk(Number(configbook.chapterpage));
            var selectpageselect = await TConfigSelector_1.TConfigSelector.findByPk(Number(configbook.chapterselect));
            var data = {};
