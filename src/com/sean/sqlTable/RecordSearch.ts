...
@Table( { tableName:"t_TT_search_record" } )
export class RecordSearch extends Model<RecordSearch>
{
    ...
    public static async checkRecord(type:string,keywd:string,ip:string):Promise<void>
    {
        ...
        var rc:RecordSearch = await RecordSearch.findOne({where:{ip:ip,type:type}});
        if(rc){
            rc.keyword = RecordSearch.addKeyWord(rc.keyword,keywd);
            rc.up_time = new Date();
        }else{
            rc = new RecordSearch();
    ...
    protected static addKeyWord(keys:string,keyword:string):string{
        ...
        arrkeys = RecordSearch.updateKeywords(arrkeys,keyword);
